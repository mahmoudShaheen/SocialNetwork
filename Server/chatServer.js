	...
	ws.on('message', function(msg){
		...
		if(message.type == "open_chat"){
			...
			s.clients.forEach( function e(client){
				...
				}else if(client.personName == ws.personName && client.toName == ws.toName){
					client.send(JSON.stringify({
						type: "msg",
						name: "You",
						data: message.data
			...
			return;
		}else if(message.type == "open_socket"){
			ws.socketID = ClientNum;
			ClientNum += 1;
			console.log("new client connected");
			getUsers(ws, message.fromname);
...
function getUsers(WS, uname){
	
	var sql = "SELECT username FROM user where username <> \"" + uname + "\" order by username;" ;
	connection.query( sql , function(err, rows){
		if(!err){
			console.log(rows);
			for( var i=0 ; i<rows.length ; i++){
				if(rows[i].username.trim() != WS.personName){
					WS.send(JSON.stringify({
						type: "users",
			...
			console.log('Error while performing Query. from getUsers function');
			console.log(sql);
		}
	});
}
